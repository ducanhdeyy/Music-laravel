                ...
                <div class="row no-gutters list-grouped">
                    ...
                    <div class="col-sm-6">
                        <div class="list-item list-overlay r mb-3" data-sr-id="220" style="visibility: visible; transform: none; opacity: 1; transition: none 0s ease 0s;">
                            <div class="media media-4x3"><a href="" class="ajax media-content" style="background-image:url({{asset('uploads/' . $album->image)}})" data-pjax-state=""></a>
                                <div class="media-action"></div>
                            </div>
                            <div class="list-content p-5">
                                <div class="list-body"><a href="" class="list-title title ajax h4 font-weight-bold" data-pjax-state="">{{$album->name}}</a><a href="" class="list-subtitle d-block text-muted subtitle ajax h-1x" data-pjax-state="">{{$album->description}}</a></div>
                            </div>
                        </div>
                    </div>
                    @endforeach
                ...
                <div class="slick slick-visible slick-arrow-top row sr-item" data-sr-id="224" style="visibility: visible; transform: none; opacity: 1; transition: none 0s ease 0s;">
                    ...
                    <div class="col-2" data-id="92570808" data-category="Pop" data-tag="Canada" data-source="https://audio-ssl.itunes.apple.com/apple-assets-us-std-000001/Music4/v4/64/3d/c1/643dc113-29d1-08de-78e2-a4ab4c3f1730/mzaf_5420937162202173294.plus.aac.p.m4a">
                        <div class="list-item slick-item r mb-3" data-sr-id="225" style="visibility: visible; transform: none; opacity: 1; transition: none 0s ease 0s;">
                            <div class="media"><a href="" class="ajax media-content" style="background-image:url({{'uploads/' .  $song->image}}" data-pjax-state=""></a>
                                <div class="media-action media-action-overlay"><button class="btn btn-icon no-bg no-shadow hide-row" data-toggle-class=""><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-heart active-danger">
                                        ...
                                        </svg></button> <div data-index="@php echo $i; $i++ @endphp" class="playpause-track1" data-url="{{'uploads/' . $song->file_url}}" data-name="{{$song->name}}" data-image="{{asset('uploads/' . $song->image)}}"><i class="fa fa-play-circle fa-3x"></i></div>
                            ...
                            <div class="list-content text-center">
                                <div class="list-body"><a href="" class="list-title title ajax h-1x" data-pjax-state="">{{$song->name}} </a><a href="" class="list-subtitle d-block text-muted subtitle ajax h-1x" data-pjax-state="">{{$song->singer->name}}</a></div>
                            </div>
                        </div>
                    </div>
                    @endforeach
                        ...
                        <div class="row list-row list-index">
                            ...
                            <div class="col-12" data-id="82924078" data-category="France" data-tag="Electronic" data-source="https://audio-ssl.itunes.apple.com/apple-assets-us-std-000001/Music/v4/fa/37/1c/fa371cea-559d-f418-506a-5fdf64bed3fe/mzaf_1505180730434746810.plus.aac.p.m4a">
                                <div class="list-item r" data-sr-id="239" style="visibility: visible; transform: none; opacity: 1; transition: none 0s ease 0s;">
                                    <div class="media"><a href="" class="ajax media-content" style="background-image:url({{asset('uploads/' . $newSong->image)}})" data-pjax-state=""></a>
                                        <div class="media-action media-action-overlay"><button class="btn btn-icon no-bg no-shadow hide-row" data-toggle-class=""><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-heart active-danger">
                                                    <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"></path>
                                                </svg></button> <div data-index="@php echo $i; $i++ @endphp" class="playpause-track1" data-url="{{asset('uploads/' . $newSong->file_url)}}" data-name="{{$newSong->name}}" data-image="{{asset('uploads/' . $newSong->image)}}"><i class="fa fa-play-circle fa-3x"></i></div>
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right text-center">
                                                @if(Auth::guard('cus')->check())
                                                    <form action="{{route('playlist.store', Auth::guard('cus')->id())}}" method="post">
                                                        @csrf
                                                        <input type="hidden" name="user_id" value="{{Auth::guard('cus')->id()}}">
                                                        <input type="hidden" name="song_id" value="{{$newSong->id}} ">
                                                        <button class="btn btn-outline-">Add to playlist</button>
                                                    </form>
                                                    <form action="{{route('download')}}" method="post">
                                                        @csrf
                                                        <input type="hidden" name="url" value="{{$newSong->file_url}}">
                                                        <input type="hidden" name="price" value="{{$newSong->price}}">
                                                        <input type="hidden" name="song_id" value="{{$newSong->id}}">
                                                        <input type="hidden" name="user_id" value="{{Auth::guard('cus')->id()}}">
                                                        <input type="hidden" name="wallet" value="{{Auth::guard('cus')->user()->wallet}}">
                                                        <button class="btn btn-outline-">{{number_format($newSong->price) }} USD <i class="ml-2 fa-solid fa-download"></i></button>
                                                    </form>
                                                @else
                                                    <form action="{{route('login.index')}}" method="">
                                                        <button class="btn btn-outline-">Add to playlist</button>
                                    ...
                                    <div class="list-content text-center">
                                        <div class="list-body"><a href="" class="list-title title ajax h-1x" data-pjax-state="">{{$newSong->name}}</a><a href="" class="list-subtitle d-block text-muted subtitle ajax h-1x" data-pjax-state="">
                                                    {{$newSong->singer->name}}</a></div>
                                    </div>
                                    <div class="list-action show-row">
                                        <div class="d-flex align-items-center">
                                            <div class="px-3 text-sm d-none d-md-block"></div><button class="btn btn-icon no-bg no-shadow" data-toggle-class=""><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-heart active-danger">
                                                    <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"></path>
                    ...
                    <div class="col-md-6">
                        <div class="heading py-2 d-flex">
                            <div>
                                ...
                                <h5 class="text-highlight sr-item" data-sr-id="245" style="visibility: visible; transform: none; opacity: 1; transition: none 0s ease 0s;">Genre</h5>
                            </div><span class="flex"></span>
                        </div>
                        ...
                        <div class="heading py-2 d-flex">
                            <div>
                                ...
                                <h5 class="text-highlight sr-item" data-sr-id="249" style="visibility: visible; transform: none; opacity: 1; transition: none 0s ease 0s;">Singer</h5>
                            </div><span class="flex"></span>
                        </div>
                        <div class="row row-sm">
                           @foreach ($singers as $singer)
                            <div class="col-4">
                                <div class="list-item r" data-sr-id="250" style="visibility: visible; transform: none; opacity: 1; transition: none 0s ease 0s;">
                                    <div class="media media-16x9"><a href="?controller=home&action=singerSong&id=<?php echo $singer['id']?>" class="ajax media-content" style="background-image:url({{asset('uploads/' . $singer->image)}})" data-pjax-state=""></a>
                                        <div class="media-action"></div>
                                    </div>
                                    <div class="list-content">
                                        <div class="list-body"><a href="" class="list-title title ajax" data-pjax-state="">{{$singer->name}}</a>
                                        </div>
