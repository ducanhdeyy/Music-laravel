                                                ...
                                                <div class="media col-4"><a href="" class="ajax media-content"
                                                    ...
                                                    <div class="media-action media-action-overlay">
                                                        ...
                                                        <div data-index="@php echo $i; $i++; @endphp"
                                                            ...
                                                            data-url="{{ asset('uploads/' . $playlist->file_url) }}"
                                                            data-name="{{ $playlist->name }}"
                                                            data-image="{{ asset('uploads/' . $playlist->image) }}"><i
                                                        ...
                                                        <div class="dropdown-menu dropdown-menu-right text-center">
                                                            <form action="{{ route('playlist.delete') }}" method="post">
                                                                ...
                                                                <button class="btn btn-outline-danger"
                                                                        ...
                                                                        data-pjax-state="">Delete   <i class="fa-solid fa-trash ml-2"></i></a>
                                                            </form>
                                                        </div>
                                        ...
                                        <div class="list-item r">
                                            <div class="media"><a href="" class="ajax media-content"
                                                ...
                                                <div class="media-action">
                                                    ...
                                                    $i++; ?>" class="playpause-track1"
                                                        data-url="{{ asset('uploads/' . $topTrack->file_url) }}"
                                                        data-name="{{ $topTrack->name }}"
                                                        data-image="{{ asset('uploads/' . $topTrack->image) }}"><i
                                                            class="fa fa-play-circle fa-3x"></i></div>
                                                    <button class="btn btn-icon no-bg no-shadow hide-row btn-more"
                                                        data-toggle="dropdown">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="16"
                                            ...
                                            <div class="list-content">
                                                <div class="list-body"><a href=""
                                                        ...
                                                        data-pjax-state=""><?php echo $topTrack['name_singer']; ?></a></div>
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
...
@section('js')
    @php
        $playlistLists = $playlists->concat($topTracks)->all();
        foreach ($playlistLists as $playlistList) {
            $playlistList->image = asset('uploads/' . $playlistList->image);
            $playlistList->file_url = asset('uploads/' . $playlistList->file_url);
        }
    @endphp

    <script>
        const playlist = '<?php echo json_encode($playlistLists); ?>';
    </script>
@endsection
